...
func main() {
	...
	// Reads the derivation's file off disk for the bound figaro and
	// formats it (default: pretty-printed JSON; -json: raw bytes).
	if len(os.Args) > 1 && (os.Args[1] == "-s" || os.Args[1] == "--search") {
		runSearch(loaded, os.Args[2:])
		return
	}
...
// segments (`["42", "cache_control"]`). The top-level key is the
// longest dotted prefix that contains no brackets — i.e. everything
// up to the first `[` or the segment immediately preceding it.
func parseChalkboardPath(s string) (string, []string, error) {
	if s == "" {
		return "", nil, fmt.Errorf("empty key")
	}
	// Find first '['; everything before it (including dotted parts) is
	// the top-level chalkboard key. If absent, the whole string is the
	// top-level key (existing behavior).
...
// the leaf to `value`. Missing intermediate objects are created.
// Non-object intermediates are overwritten with a fresh object.
...
//
// Missing intermediates are a no-op (returns current unchanged) — the
// key wasn't there, so deletion is vacuously satisfied.
func deepDeleteJSON(current json.RawMessage, path []string) (json.RawMessage, bool, error) {
	if len(path) == 0 {
		return nil, true, nil // caller asked to delete top-level
	}
	if len(current) == 0 || string(current) == "null" {
	...
	if !deepDeleteWalk(obj, path) {
		return current, false, nil // path didn't exist
	}
	if len(obj) == 0 {
		return nil, true, nil // empty: tell caller to drop the top-level key
	}
	out, err := json.Marshal(obj)
	if err != nil {
...
// deepDeleteWalk recurses into obj, deletes path[len(path)-1] from the
// terminal map, and prunes any empty parent maps left behind. Returns
...
//
// Each key may be a simple top-level chalkboard name, or a path with
// `.field` and `[index]` segments — e.g. `system.tags[42].cache_control`.
// In path mode, the existing top-level value is fetched, the leaf is
// pruned, and the resulting top-level value is sent back as a Set (or
// as a Remove if pruning collapses it to empty).
func runUnset(loaded *config.Loaded) {
	...
	for _, keyArg := range args {
		...
		current := mustFetchChalkboardKey(loaded, top)
		if len(current) == 0 {
			continue // key wasn't there — nothing to unset
		}
...
// (whatever the derivation wrote, untouched).
//
// Usage: figaro -s <alias> [-json]
//
//	figaro -s usage          # human-readable
//	figaro -s usage -json    # raw JSON
func runSearch(loaded *config.Loaded, args []string) {
	...
	// Compute the destination path. We need the provider name for
	// dynamic-filename derivations (e.g. translator). Read it from
	// the figaro's chalkboard snapshot via the connected client.
	ep := transport.Endpoint{Scheme: r.Endpoint.Scheme, Address: r.Endpoint.Address}
	fcli, err := figaro.DialClient(ep, nil)
	if err != nil {
		die("connect figaro: %s", err)
	...
	// Pretty-print: re-marshal indented.
	var any interface{}
