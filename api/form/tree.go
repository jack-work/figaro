// Immutable AVL tree for form snapshots.

package form

import (
	"encoding/json"
	"iter"
	"slices"
	"sort"
	"strings"
)

// ptree is an immutable, string-keyed ordered map of JSON values. The zero
// ptree is a valid empty tree. Every method leaves the receiver untouched.
type ptree struct {
	root *node
}

// node is one SEGMENT of a path. It holds either a leaf value or a subtree,
// never both: a key whose own name contains a dot is a single segment at its
// level, which is what keeps skills.howto and skills.howto.md apart.
type node struct {
	key    string
	value  Value
	kids   ptree
	branch bool

	// terminal marks a node that is the END of a key someone wrote, rather
	// than a segment invented by splitting one. A later key may not descend
	// through it: skills.howto names a skill, so skills.howto.md names a
	// second skill beside it and not a field inside the first.
	terminal    bool
	left, right *node
	height      int
	size        int
}

// treeFromMap builds a tree from a plain map, in O(n log n) for the sort plus
// O(n) for the build: exactly n nodes, no rotations, no rebalancing garbage.
func treeFromMap(m map[string]json.RawMessage) ptree {
	entries := make([]treeEntry, 0, len(m))
	for k, v := range m {
		entries = append(entries, treeEntry{key: k, value: NewValue(v)})
	}
	return treeFromEntries(entries)
}

// treeEntry is a key/value pair for bulk construction.
type treeEntry struct {
	key   string
	value Value
}

// treeFromEntries sorts entries by key and builds a perfectly balanced tree
// bottom-up. Keys must be unique (they always are: they come from a map or a
// JSON object). Bulk-building beats n repeated Sets by a wide margin: no
// comparisons past the sort, no rotations, and no intermediate path copies -
// and it is on the aria-load and WAL-replay paths, so it matters.
func treeFromEntries(entries []treeEntry) ptree {
	slices.SortFunc(entries, func(a, b treeEntry) int { return strings.Compare(a.key, b.key) })
	return ptree{root: buildBalanced(entries)}
}

func buildBalanced(entries []treeEntry) *node {
	if len(entries) == 0 {
		return nil
	}
	mid := len(entries) / 2
	return makeNode(entries[mid].key, entries[mid].value,
		buildBalanced(entries[:mid]), buildBalanced(entries[mid+1:]))
}

// Len returns the number of entries. O(1).
func (t ptree) Len() int { return nodeSize(t.root) }

// Get looks up key. O(log n).
func (t ptree) Get(key string) (Value, bool) {
	for n := t.root; n != nil; {
		switch {
		case key < n.key:
			n = n.left
		case key > n.key:
			n = n.right
		default:
			return n.value, true
		}
	}
	return Value{}, false
}

// Has reports whether key is present.
func (t ptree) Has(key string) bool {
	_, ok := t.Get(key)
	return ok
}

// Set returns a tree containing key bound to value. The receiver is unchanged.
func (t ptree) Set(key string, value Value) ptree {
	root := setNode(t.root, key, value)
	if root == t.root {
		return t
	}
	return ptree{root: root}
}

// Delete returns a tree without key. An absent key returns the receiver.
func (t ptree) Delete(key string) ptree {
	root, found := deleteNode(t.root, key)
	if !found {
		return t
	}
	return ptree{root: root}
}

// Range visits entries in lexical key order, stopping when yield returns false.
func (t ptree) Range(yield func(key string, value Value) bool) {
	rangeNode(t.root, yield)
}

// All iterates entries in lexical key order.
func (t ptree) All() iter.Seq2[string, Value] {
	return func(yield func(string, Value) bool) { rangeNode(t.root, yield) }
}

// setEntry replaces the whole entry at key, preserving nothing of the old.
func setEntry(n *node, e *node) *node {
	if n == nil {
		return &node{key: e.key, value: e.value, kids: e.kids, branch: e.branch, terminal: e.terminal, height: 1, size: 1}
	}
	switch {
	case e.key < n.key:
		return balance(cloneWith(n, setEntry(n.left, e), n.right))
	case e.key > n.key:
		return balance(cloneWith(n, n.left, setEntry(n.right, e)))
	}
	out := *n
	out.value, out.kids, out.branch = e.value, e.kids, e.branch
	out.terminal = out.terminal || e.terminal
	return &out
}

// cloneWith copies n with new children, keeping its own entry.
func cloneWith(n *node, left, right *node) *node {
	out := *n
	out.left, out.right = left, right
	out.height = 1 + max(nodeHeight(left), nodeHeight(right))
	out.size = 1 + nodeSize(left) + nodeSize(right)
	return &out
}

func setNode(n *node, key string, value Value) *node {
	if n == nil {
		return makeNode(key, value, nil, nil)
	}
	switch {
	case key < n.key:
		left := setNode(n.left, key, value)
		if left == n.left {
			return n
		}
		return balance(cloneWith(n, left, n.right))
	case key > n.key:
		right := setNode(n.right, key, value)
		if right == n.right {
			return n
		}
		return balance(cloneWith(n, n.left, right))
	default:
		if n.value.Equal(value) {
			return n
		}
		return makeNode(key, value, n.left, n.right)
	}
}

func deleteNode(n *node, key string) (*node, bool) {
	if n == nil {
		return nil, false
	}
	switch {
	case key < n.key:
		left, found := deleteNode(n.left, key)
		if !found {
			return n, false
		}
		return balance(cloneWith(n, left, n.right)), true
	case key > n.key:
		right, found := deleteNode(n.right, key)
		if !found {
			return n, false
		}
		return balance(cloneWith(n, n.left, right)), true
	}
	if n.left == nil {
		return n.right, true
	}
	if n.right == nil {
		return n.left, true
	}
	successor := minNode(n.right)
	return balance(makeNode(successor.key, successor.value, n.left, deleteMin(n.right))), true
}

func deleteMin(n *node) *node {
	if n.left == nil {
		return n.right
	}
	return balance(cloneWith(n, deleteMin(n.left), n.right))
}

func minNode(n *node) *node {
	for n.left != nil {
		n = n.left
	}
	return n
}

func rangeNode(n *node, yield func(string, Value) bool) bool {
	if n == nil {
		return true
	}
	return rangeNode(n.left, yield) && yield(n.key, n.value) && rangeNode(n.right, yield)
}

func makeBranch(key string, kids ptree, left, right *node) *node {
	n := &node{key: key, kids: kids, branch: true, left: left, right: right}
	n.height = 1 + max(nodeHeight(left), nodeHeight(right))
	n.size = 1 + nodeSize(left) + nodeSize(right)
	return n
}

func makeNode(key string, value Value, left, right *node) *node {
	return &node{
		key: key, value: value, left: left, right: right,
		height: 1 + max(nodeHeight(left), nodeHeight(right)),
		size:   1 + nodeSize(left) + nodeSize(right),
	}
}

func balance(n *node) *node {
	factor := nodeHeight(n.left) - nodeHeight(n.right)
	if factor > 1 {
		if nodeHeight(n.left.left) < nodeHeight(n.left.right) {
			n = cloneWith(n, rotateLeft(n.left), n.right)
		}
		return rotateRight(n)
	}
	if factor < -1 {
		if nodeHeight(n.right.right) < nodeHeight(n.right.left) {
			n = cloneWith(n, n.left, rotateRight(n.right))
		}
		return rotateLeft(n)
	}
	return n
}

func rotateLeft(n *node) *node {
	r := n.right
	return cloneWith(r, cloneWith(n, n.left, r.left), r.right)
}

func rotateRight(n *node) *node {
	l := n.left
	return cloneWith(l, l.left, cloneWith(n, l.right, n.right))
}

func nodeHeight(n *node) int {
	if n == nil {
		return 0
	}
	return n.height
}

func nodeSize(n *node) int {
	if n == nil {
		return 0
	}
	return n.size
}

// --- paths -----------------------------------------------------------------

// entry finds the node a dotted path names, longest segment first so a key
// whose own name contains a dot resolves to itself.
func (t ptree) entry(path string) (*node, bool) {
	cur, rest := t, path
	for {
		n := lookupLongest(cur.root, rest)
		if n == nil {
			return nil, false
		}
		if len(n.key) == len(rest) {
			return n, true
		}
		if !n.branch {
			return nil, false
		}
		cur, rest = n.kids, rest[len(n.key)+1:]
	}
}

// lookupLongest returns the entry whose key is rest, or the longest key that
// is a dotted prefix of it.
func lookupLongest(n *node, rest string) *node {
	var best *node
	for n != nil {
		switch {
		case rest < n.key:
			n = n.left
		case rest > n.key:
			if len(n.key) < len(rest) && rest[len(n.key)] == '.' && rest[:len(n.key)] == n.key {
				if best == nil || len(n.key) > len(best.key) {
					best = n
				}
			}
			n = n.right
		default:
			return n
		}
	}
	return best
}

// setPath rebuilds only the nodes on the path, sharing every subtree it does
// not touch. Where a segment lands on a leaf, the rest of the path stays whole.
func (t ptree) setPath(segs []string, v Value) ptree {
	if len(segs) == 0 {
		return t
	}
	head := segs[0]
	if len(segs) == 1 {
		e := leafOrBranch(head, v)
		e.terminal = true
		return ptree{root: setEntry(t.root, e)}
	}
	cur := lookupExact(t.root, head)
	if cur != nil && cur.terminal {
		// The segment names something a caller already wrote, so the rest of
		// this key stays whole beside it.
		return ptree{root: setEntry(t.root, leafOrBranch(joinSegs(segs), v))}
	}
	var kids ptree
	if cur != nil {
		kids = cur.kids
	}
	kids = kids.setPath(segs[1:], v)
	return ptree{root: setEntry(t.root, &node{key: head, kids: kids, branch: true})}
}

func (t ptree) deletePath(segs []string) (ptree, bool) {
	if len(segs) == 0 {
		return t, false
	}
	head := segs[0]
	if len(segs) == 1 {
		root, found := deleteNode(t.root, head)
		return ptree{root: root}, found
	}
	if n := lookupExact(t.root, joinSegs(segs)); n != nil {
		root, found := deleteNode(t.root, joinSegs(segs))
		return ptree{root: root}, found
	}
	cur := lookupExact(t.root, head)
	if cur == nil || !cur.branch {
		return t, false
	}
	kids, found := cur.kids.deletePath(segs[1:])
	if !found {
		return t, false
	}
	if kids.root == nil {
		root, _ := deleteNode(t.root, head)
		return ptree{root: root}, true
	}
	return ptree{root: setEntry(t.root, &node{key: head, kids: kids, branch: true})}, true
}

func lookupExact(n *node, key string) *node {
	for n != nil {
		switch {
		case key < n.key:
			n = n.left
		case key > n.key:
			n = n.right
		default:
			return n
		}
	}
	return nil
}

func joinSegs(segs []string) string {
	out := segs[0]
	for _, s := range segs[1:] {
		out += "." + s
	}
	return out
}

// leaves walks every addressable leaf in key order, yielding its dotted path.
func (t ptree) leaves(prefix string, yield func(string, Value) bool) bool {
	return rangeEntries(t.root, func(n *node) bool {
		path := n.key
		if prefix != "" {
			path = prefix + "." + n.key
		}
		if !n.branch {
			return yield(path, n.value)
		}
		return n.kids.leaves(path, yield)
	})
}

func rangeEntries(n *node, yield func(*node) bool) bool {
	if n == nil {
		return true
	}
	if !rangeEntries(n.left, yield) {
		return false
	}
	if !yield(n) {
		return false
	}
	return rangeEntries(n.right, yield)
}

// leafOrBranch makes the entry a value stands for. An object becomes a
// branch, so its fields are addressable and stay so across a reload: the
// storage shape and the read shape must agree or a board would mean one thing
// when written and another when opened.
func leafOrBranch(key string, v Value) *node {
	members, isObj := v.members()
	if !isObj {
		return &node{key: key, value: v}
	}
	var kids ptree
	names := make([]string, 0, len(members))
	for k := range members {
		names = append(names, k)
	}
	sort.Strings(names)
	for _, k := range names {
		kids = ptree{root: setEntry(kids.root, leafOrBranch(k, members[k]))}
	}
	return &node{key: key, kids: kids, branch: true}
}
