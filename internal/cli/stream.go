...
// frame and the alert's TTL are functions of time, and that is now ALL it is
// for.
//
...
//
// Safe to invoke concurrently with any prior fetch: the last completion wins,
...
func (in *interactiveInput) selectNodeKey(delta int, ev keyEvent) keyVerdict {
	...
	if ev.shift {
		...
		in.lt.transcriptSelect(delta, ev.alt)
	}
	in.mu.Unlock()
	in.pageWanted = true
	return keyHandled
