...
// countingWriter is the wire: how many bytes a frame actually costs the
// terminal. On a local terminal this is nearly free; over ssh or through tmux
// it is the whole latency budget, which is why it deserves its own number
// alongside ns/op.
...
func (c *countingWriter) reset() { c.n, c.writes = 0, 0 }

// countingTranscript is heavyTranscript with a writer that counts instead of
// discarding. (heavyTranscript itself is off limits: it lives in the file the
// scroll benchmarks own.)
...
func BenchmarkTranscriptFrameBytes(b *testing.B) {
	for _, outputLines := range []int{20, 200} {
		b.Run(fmt.Sprintf("out%d", outputLines), func(b *testing.B) {
			...
			b.StopTimer()
			if b.N > 0 {
				b.ReportMetric(float64(cw.n)/float64(b.N), "B/frame")
				b.ReportMetric(float64(cw.writes)/float64(b.N), "writes")
			}
		})
	}
}
...
func BenchmarkTranscriptEnterBytes(b *testing.B) {
	...
	b.StopTimer()
	if b.N > 0 {
		b.ReportMetric(float64(cw.n)/float64(b.N), "B/enter")
	}
}
