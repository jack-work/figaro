...
// One line per form, board first, keys joined -- not one line per key.
func TestFormDeltaLinesGroupByForm(t *testing.T) {
	...
	if len(lines) != 2 {
		t.Fatalf("want one line per form, got %d: %q", len(lines), lines)
	}
	if !strings.Contains(lines[0], "board") || !strings.Contains(lines[0], `phase="canary"`) {
		t.Fatalf("the board draws first, as \"board\": %q", lines[0])
	}
	if !strings.Contains(lines[1], "@f1") || !strings.Contains(lines[1], "sha=") || !strings.Contains(lines[1], "status=") {
		t.Fatalf("the studied form's keys join on one line: %q", lines[1])
	}
}

// A removed key says -key; a deleted form says deleted; they are not the
// same thing and must not read the same.
func TestFormDeltaLinesRemovedVersusDeleted(t *testing.T) {
	...
	if len(removed) != 1 || !strings.Contains(removed[0], "-brief") {
		t.Fatalf("a removed key draws as -key: %q", removed)
	}
	deleted := formDeltaLines(map[string]livedoc.FormDelta{
		"@f1": fd("@f1", livedoc.FormStudied, livedoc.FormDeleted, ""),
	}, 120, false)
	if len(deleted) != 1 || !strings.Contains(deleted[0], "deleted") {
...
// Collapsed caps values and stays on one row; expanded shows them whole,
// one key per row. deltasExpandable is the difference between the two.
func TestFormDeltaExpansion(t *testing.T) {
	...
	collapsed := formDeltaLines(deltas, 60, false)
	if len(collapsed) != 1 {
		t.Fatalf("collapsed is one row: %q", collapsed)
	}
	expanded := formDeltaLines(deltas, 300, true)
	if len(expanded) != 2 {
		t.Fatalf("expanded is one row per key: %q", expanded)
	}
	whole := false
