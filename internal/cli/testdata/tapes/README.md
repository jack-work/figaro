...
| `stream-eager.tape` | recorded, anthropic direct with `system.eager_tool_streaming = true` | PASS. The arguments STREAM: 11 input patches over t=7.8→15.1s |
| `stream-buffered.tape` | recorded, same route with the flag off | PASS. The arguments DO NOT stream: one 43-byte set at t=19.1s, the API having buffered each parameter until it was whole |
