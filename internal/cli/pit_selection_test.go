...
func idRow(text, id string) drawerRow { return drawerRow{text: text, yank: text, id: id} }
...
func TestPickerCursorIsDerivedFromRowsNotFromBirth(t *testing.T) {
	p := newPicker([]drawerRow{staticRow("   (none)")})
	if p.hasCursor() {
		t.Fatalf("a list of chrome has a cursor: %d", p.cursor)
	}
	p.setRows([]drawerRow{idRow("first", "1"), idRow("second", "2")}, "")
	if !p.hasCursor() {
	...
	p.setRows([]drawerRow{staticRow("   (none)")}, "")
	if p.hasCursor() {
...
func TestPickerSetRowsKeepsSelectionAcrossReorder(t *testing.T) {
	rows := []drawerRow{idRow("a", "1"), idRow("b", "2"), idRow("c", "3")}
	p := newPicker(rows)
	p.pick(2)
	if row, _ := p.selected(); row.id != "3" {
		t.Fatalf("setup: cursor on %+v", row)
	}
	p.setRows([]drawerRow{idRow("c", "3"), idRow("a", "1"), idRow("b", "2")}, "")
	if row, _ := p.selected(); row.id != "3" {
...
func TestQueuePitGainsCursorWhenTheFetchLands(t *testing.T) {
	var d drawer
	d.showList("queue", "", []drawerRow{staticRow("   (none)")})
	if _, ok := d.selected(); ok {
		t.Fatal("an empty queue has a selection")
	}
	d.replaceRows([]drawerRow{idRow("hello", "7"), idRow("world", "8")}, "")
	row, ok := d.selected()
	if !ok || row.id != "7" {
...
func (v *fakeItemView) Items(width int) []drawerRow { return v.rows }
...
func TestLivePitHasASelectionTheVerbsCanSee(t *testing.T) {
	...
	v.rows = append([]drawerRow{staticRow("form abc · v2"), idRow("aria_id: y", "aria_id")}, v.rows[1:]...)
	d.refreshLive(80)
	if row, _ := d.selected(); row.id != "skills" {
...
func TestPitClosingReleasesTheHostedView(t *testing.T) {
	v := &closingView{fakeItemView: fakeItemView{rows: []drawerRow{idRow("mantra: x", "mantra")}}}
	var d drawer
	d.showLive("form show", v)
	d.close()
	if v.closed != 1 {
	...
	w := &closingView{fakeItemView: fakeItemView{rows: []drawerRow{idRow("a", "a")}}}
	d.showLive("form show", w)
	d.showLive("form listen", &closingView{})
	if w.closed != 1 {
