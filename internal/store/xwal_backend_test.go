...
func TestTurnJournalPendingCheckpointFollowsContinuation(t *testing.T) {
	for _, interior := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := journal.Checkpoint(target, []byte(`{"generation":1}`)); err != nil {
				t.Fatal(err)
			}
			if err := journal.Checkpoint(target, []byte(`{"generation":2}`)); err != nil {
				t.Fatal(err)
			}
			...
			var cont, alt string
			if interior {
				cont, alt, err = b.ForkAt(conv, tail.FigaroLT-1)
			} else {
				cont, alt, err = b.Fork(conv)
			}
			if err != nil {
			...
			contJournal, err := b.OpenTurnJournal(cont)
			if err != nil {
				t.Fatal(err)
			}
			got, ok, err := contJournal.Latest(target)
			if err != nil || !ok || string(got) != `{"generation":2}` {
				t.Fatalf("continuation checkpoint = %q, %v, %v", got, ok, err)
			}
			altJournal, err := b.OpenTurnJournal(alt)
			if err != nil {
				t.Fatal(err)
			}
			if got, ok, err := altJournal.Latest(target); err != nil || ok {
				t.Fatalf("alternative inherited pending checkpoint = %q, %v, %v", got, ok, err)
			}
		})
	}
}
