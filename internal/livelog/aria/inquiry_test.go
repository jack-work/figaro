...
func TestClientAlwaysHasTheQuestion(t *testing.T) {
	...
	drive := func(s *Server) {
		...
		s.OpenTurn(1)
		s.Update([]livedoc.Node{{Type: livedoc.NodeProse, Markdown: "answering"}}, 0)
		s.Update([]livedoc.Node{{Type: livedoc.NodeProse, Markdown: "answering more"}}, 0)
		s.Update([]livedoc.Node{{Type: livedoc.NodeProse, Markdown: "answering yet more"}}, 0)
		s.Close()
		s.Seal(nil)
	}
...
func TestLateJoinerRecoversTheQuestionFromARead(t *testing.T) {
	...
	s.Update([]livedoc.Node{{Type: livedoc.NodeProse, Markdown: "more"}}, 0)
...
func TestQuestionIsNotRestatedOnEveryFrame(t *testing.T) {
	drive := func(q string) (pushed, frames, carrying int) {
		...
		s.OpenTurn(1)
		for i := 0; i < 40; i++ {
			s.Update([]livedoc.Node{{Type: livedoc.NodeProse, Markdown: strings.Repeat("x", i+1)}}, 0)
		}
		s.Close()
		return
	}
