...
func TestApplyThinking_Budget(t *testing.T) {
	...
	applyThinking(&p, chalkboard.FromMap(map[string]json.RawMessage{"system.thinking_budget": json.RawMessage("2048")}), budgetModel)
	...
	applyThinking(&p, chalkboard.FromMap(map[string]json.RawMessage{"system.thinking_budget": json.RawMessage("500")}), budgetModel)
...
func TestApplyThinking_Adaptive(t *testing.T) {
	...
	applyThinking(&p, chalkboard.FromMap(map[string]json.RawMessage{"system.thinking_budget": json.RawMessage("2048")}), "claude-opus-4-7")
	...
	p = anthropic.MessageNewParams{MaxTokens: 32000}
	applyThinking(&p, chalkboard.FromMap(map[string]json.RawMessage{"system.thinking_effort": json.RawMessage(`"xhigh"`)}), "claude-opus-4-8")
...
func TestApplyThinking_Off(t *testing.T) {
	for _, model := range []string{budgetModel, "claude-opus-4-7"} {
		for _, snap := range []chalkboard.Snapshot{
			chalkboard.Snapshot{},
			chalkboard.FromMap(map[string]json.RawMessage{"system.thinking_budget": json.RawMessage("0")}),
		} {
			p := anthropic.MessageNewParams{MaxTokens: 1000}
