...
// TestSpeculativeDispatch_BatchStartBeforeAnyToolStart preserves the
// CLI's wire-order invariant: MethodToolBatchStart must arrive before
// any MethodToolStart/MethodToolEnd for the batch, even though tools
// may have already executed speculatively.
func TestSpeculativeDispatch_BatchStartBeforeAnyToolStart(t *testing.T) {
	zero := time.Now()
	...
	a := figaro.NewAgent(figaro.Config{
		ID:         "spec-003",
		SocketPath: "/tmp/spec-test-3.sock",
		Provider:   prov,
		Tools:      reg,
		Chalkboard: cb,
	})
	...
	var sawBatchStart, sawToolStartBeforeBatch atomic.Bool
	var firstBatchStartAt, firstToolStartAt int
	var ord int
	timeout := time.After(5 * time.Second)
	var sawDone bool
	for !sawDone {
		...
		case n := <-ch:
			...
			case rpc.MethodToolBatchStart:
				if !sawBatchStart.Swap(true) {
					firstBatchStartAt = ord
				}
			case rpc.MethodToolStart:
				if firstToolStartAt == 0 {
					firstToolStartAt = ord
				}
				if !sawBatchStart.Load() {
					sawToolStartBeforeBatch.Store(true)
				}
			case rpc.MethodDone:
				sawDone = true
			}
	...
	assert.True(t, sawBatchStart.Load(), "expected MethodToolBatchStart")
	assert.False(t, sawToolStartBeforeBatch.Load(),
		"MethodToolStart must not precede MethodToolBatchStart")
