...
func TestNodes_Summary_FromFn(t *testing.T) {
	...
	nodes := Nodes([]message.Message{assistant(invoke("t1", "bash", "ls -la"))}, nil, nil, sum, nil)
...
func TestNodes_Summary_GenericFallback_NilFn(t *testing.T) {
	...
	nodes := Nodes([]message.Message{assistant(inv)}, nil, nil, nil, nil)
...
func TestNodes_Summary_GenericFallback_EmptyReturn(t *testing.T) {
	...
	nodes := Nodes([]message.Message{assistant(inv)}, nil, nil, sum, nil)
