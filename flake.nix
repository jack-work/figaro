{
  ...
  inputs = {
    ...
    # figwal is IN THIS TREE, at internal/store/{segment,disk,log,xwal},
    # so it is neither a module requirement nor a flake input and no
    # version, pseudo-version or vendorHash bookkeeping attends a change
    # to it. The vendorHash below still covers the remaining external
    # dependencies; reset it from the "got:" value nix prints when one of
    # THOSE moves.
  };
    ...
    in
    {
      overlays.default = final: prev: {
        figaro = final.buildGoModule rec {
          ...
          version = "0.27.0";
          src = self;
          vendorHash = "sha256-JPDuMTt5j/eYjk6M8zobIDbDSftTYn2vJha9ZomwxDQ=";
        ...
        mkFigaroShell = {
          ...
          mkHushKnob = value:
            ...
            ''
              ...
              # Test-only autogenerated passphrase: unlocks this isolated dev
              # hush without an interactive prompt, so a detached daemon (e.g.
              # the one `figaro ls` spawns) doesn't hang on first run. figaro
              # honors FIGARO_HUSH_PASSPHRASE only when FIGARO_HUSH_DIR is set,
              # so it can never touch the real keystore. Stable per dev root
              # (the identity it creates must be unlockable on re-entry);
              # removed when you tear the dev root down (rm -rf $FIGARO_DEV_ROOT).
              if [ ! -f "$FIGARO_DEV_ROOT/hush-passphrase" ]; then
