...
- **A plain API key**: `api_key` in the provider config today, and
  `api_key_file` (`auth.FileValue`, in flight on the `figaro` branch) for
  servers — no impersonation anywhere on the path. Metered
