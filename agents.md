...
These hold the system up. Breaking them produces races, lost messages, or silent corruption.
...
14. **Translator catch-up is delta work, and the read that follows it holds nothing.** In either direction, normal synchronization is O(untranslated messages), usually one or two. O(total history) cache scans, copies, or lookups are a design failure; full walks are reserved for explicit fingerprint invalidation such as a model change. The READ side is bounded separately and differently: assembling a request DOES walk the whole conversation -- eeec2bc0 ruled that the walk replaces the retention, and nothing removes the walk -- but it walks it (`store.Scan`, `provider.TranslationRows`, a `provider.RowSeq` into the streamed body) and never materializes it. A `[]json.RawMessage` of the whole history on the send path is the same design failure as a whole-history scan, measured: 87% of a 461MB live heap, in one in-flight send, at 0.29.1.
